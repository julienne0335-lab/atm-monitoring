...
### 2. DB 설정
```bash
# db_example.py를 db.py로 복사
cp db_example.py db.py
```
`db.py`를 열어 `password` 항목을 본인 MariaDB 비밀번호로 수정하세요.  
...
| 권한 | 로그인 ID | 비밀번호 |
|------|-----------|---------|
| 슈퍼관리자 | (sql INSERT 파일 참고) | (sql INSERT 파일 참고) |
| 일반관리자 | (sql INSERT 파일 참고) | (sql INSERT 파일 참고) |

## 업무 규칙 요약
| 규칙 | 내용 |
|------|------|
| BR-11 | 지점장애 등록 시 소속 ATM 전체에 장애로그 자동 생성 |
| BR-12 | 은행장애 등록 시 소속 전체 ATM에 장애로그 자동 생성 |
| BR-13 | 슈퍼관리자는 자기 은행 장애만 등록·처리 가능 |
| BR-14 | 일반관리자는 자기 지점 장애만 등록·처리 가능 |
