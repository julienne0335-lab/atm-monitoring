...
def get_transaction_list(is_super, branch_id,
                         ...
                         date_from=None, date_to=None, page=1, bank_id=None):
    """
    ...
    try:
        ...
        total = transaction_dao.count_all(
            conn, branch_id=bid, bank_id=bank_fid,
...
def get_stats(is_super, branch_id, bank_id=None, date_from=None, date_to=None):
    """
    ...
    try:
        return {
            "branch_stats": transaction_dao.find_branch_stats(conn, bid, bank_id=bank_fid,
                                                              date_from=date_from, date_to=date_to),
            "type_stats":   transaction_dao.find_type_stats(conn, bid, bank_id=bank_fid,
                                                            date_from=date_from, date_to=date_to),
            "top_atms":     transaction_dao.find_top_atms(conn, bid, bank_id=bank_fid,
                                                          date_from=date_from, date_to=date_to),
        }
    finally:
        conn.close()
...
def get_today_stats(is_super, branch_id, bank_id=None):
    """
    ...
    try:
        bid      = None if is_super else branch_id
        bank_fid = bank_id if is_super else None
        return transaction_dao.find_today_stats(conn, branch_id=bid, bank_id=bank_fid)
    finally:
        conn.close()
